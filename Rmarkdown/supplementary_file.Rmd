...
Displaying graphs of representative species on a phylogenetic tree is commonly used to compare morphological differences. ETE3 [@ETE3] and iTOL [@iTOL] support linking external images to any tree node. In addition to link external images, *ggtreeExtra* supports querying silhouette images directly from the PhyloPic database^[<http://phylopic.org/>] and allows aesthetic mapping to fill the color and rescale the size of the images using tree associated data variable. Moreover, with the aids of the *ggpmisc* package [@ggpmisc], subplots that describes taxon specific statistical information can be embeded as insets in *ggtreeExtra*.
...
The *ggtreeExtra* package was developed based on the grammar of graphic [@Wilkinson2012]. It allows users mapping variables (*e.g.* abundance of species, length of genome, sampling location) of the associated data to the visual attributes (*e.g.* color, size, shape) of the geometric objects (*e.g.* bar chart, point, boxplot) on rectangular or circular phylogenetic tree (Fig. \ref{fig:OverView}). Users are free to combine different geometric layers to create complex tree annotation. We present many examples to elucidate how to use *ggtree* and *ggtreeExtra* to visualize data on phylogenetic trees in the online book^[<http://yulab-smu.top/treedata-book>].
