...
ETE3\tPython,package\tcircular,rectangular\tHeat map,scatter plot,simple bar chart,stacked bar chart,line plot,pie chart,image plot,inset plot,protein domain,MSA\tYes\tadd\tNo\tprogramming,and command,line\n
...
# Heat map, using geom_fruit to link geom_tile
p2 <- p1 +
      geom_fruit(
          data=dat2,
          geom=geom_tile,
...
The *ggtreeExtra* package enable us to link *ggtree* [@ggtree1] and geometric layers defined in the *ggplot2* community (Tab. \ref{tab:geom} and Fig. \ref{fig:OverView}). Different annotation layers to visualize high-dimensional associated data sets can be combined freely to create complex tree graphic. As demonstrated in Fig \@ref{fig:HMP}, visualizing a data graphic layer is simple and multiple layers can be added progressively in *ggtreeExtra*. 


Here, we reproduce Fig 2 of [@GraPhlAn]. The phylogenetic tree was built using a part of 3737 microbes [@PhyloPhlAn]. The associated data sets were extracted from GraPhlAn configure file. They contains information of present or not and type of ATP synthesis genes, the type and capability of fatty acid metabolism and the lengths of microbes genomes. A heatmap layer was added to display the presence of the two types (F-type and V/A-type) of ATPase (discrete variable). Another heatmap was added to visualize different types (as different colors) and capability (as transparency of the heatmap) of fatty acid metabolism. Finally a bar chart colored by phylum information was employed to represent the lengths of the genomes. These layers were added progressively to the phylogenetic tree decorated with different symbolic points to indicate phylum information of the microbes, and background colors and labels for different clades (Fig. \ref{fig:KEGG}).  
 


Notably, Proteobacteria and Spirochaetes were set to identical color in the original configure file [@GraPhlAn]. Since the phylum information of all the tips was replaced by color code, we cannot separate the tips from Proteobacteria and Spirochaetes. This is a drawback of the configure file of GraPhlAn [@GraPhlAn] as it mixes associated data with graphic setting. Some of the information was replaced by visual values and it is tedious, error-prone and sometimes not possible to converted back to original data. 
...
The *ggtreeExtra* is an extension of the *ggtree* package and it takes all the advantages of *ggtree* (for tree visualization and annotation) and the *ggplot2* community (for data layers). Compare to other tools, *ggtreeExtra* supports more visualization methods implemented as graphic layers in different *ggplot2* extension packages and the combination of these layers creates more possibility to produce complex tree graphics. The *ggtreeExtra* is more powerful to explore phylogenetic data. It is a general tool and can be applied to different disciplines.  
