...
```{r, echo=FALSE, message=FALSE, warning=FALSE,  fig.width=7.56, fig.height=3.28, error=FALSE, fig.cap='(ref:comparison)', results='asis', out.width = '52%', compare}
require(kableExtra)

x <- "ggtreeExtra\tR,package\tcircular,inward circular,rectangular,slanted,ellipse,round rectangular,radial\tHeat map,scatter plot,simple bar chart,patter bar,stacked bar chart,dodged bar chart,boxplot,pattern boxplot,violin,dot intervals plot,density plot,line plot,pie chart,image plot,inset plot\tYes\tadd,modify,delete\tYes\tprogramming,and command,line\n
GraPhlAn\tPython,package\tcircular\tHeat map,scatter plot,simple bar chart\tYes\tadd\tNo\tcommand line with configure file\n
ETE3\tPython,package\tcircular,rectangular\tHeat map,scatter plot,simple bar chart,stacked bar chart,line plot,pie chart,image plot,inset plot,protein domain,MSA[note]\tYes\tadd\tNo\tprogramming,and command,line\n
...
The *ggtreeExtra* package enables us to link *ggtree* [@ggtred1] and geometric layers defined in the *ggplot2* community (Tab. \ref{tab:geom} and Fig. \ref{fig:OverView}). Different annotation layers to visualize high-dimensional associated data sets can be combined freely to create complex tree graphic. As demonstrated in Fig \@ref{fig:HMP}, visualizing a data graphic layer is simple and multiple layers can be added progressively in *ggtreeExtra*. 
