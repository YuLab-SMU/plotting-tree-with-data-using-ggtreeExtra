...
```{r, echo=FALSE, message=FALSE, warning=FALSE,  fig.width=6.8, fig.height=3.8, error=FALSE, fig.cap='(ref:comparison)', dev="CairoPNG", dpi=300, results='asis', out.width = '56%', compare}
require(kableExtra)

x <- "ggtreeExtra\tR,package\tcircular,inward circular,rectangular,slanted,ellipse,round rectangular,radial\tHeat map,scatter,simple bar,patter bar,stacked bar,dodged bar,box,pattern box,violin,dot intervals plot,density plot,line plot,pie,image plot,insert plot\tYes\tadd,modify,delete\tYes\tprogramming,and command line\n
GraPhlAn\tPython,package\tcircular\tHeat map,scatter,simple bar\tYes\tadd\tNo\tcommand line with configure file\n
ETE3\tPython,package\trectangular\tHeat map,scatter,simple bar,stacked bar,box,pie,image plot\tYes\tadd\tNo\tprogramming,and command line\n
iTOL\tWeb,tool\tcircular,rectangular,slanted,ellipse,inward circular\tHeat map,scatter,simple bar,stacked bar,dodged bar,box,pie,image plot\tYes\tadd\tNo\tinteractive,(mouse click,with,configure file)\n
Microreact\tWeb,tool\tcircular,rectangular\tHeat map,scatter\tYes\tadd\tNo\tinteractive,(mouse click,and command line with,configure file)\n
Evolview\tWeb,tool\tcircular,rectangular,slanted,round rectangular\tHeat map,scatter,simple bar,stacked bar\tYes\tadd\tNo\tinteractive,(mouse click with,configure file)\n"
...
p3 <- ggplot(data=dy3) +
      geom_col(aes(x=value, y=number), fill="grey68") +
...
```{r, fig.width=8, fig.height=8, error=FALSE, warning=FALSE, message=FALSE, fig.cap='(ref:ggridges)', dev="CairoPNG", dpi=300, ggridges}
library(ggtree)
library(ggplot2)
library(ggtreeExtra)
library(patchwork)
...
(ref:subplot) Using subplots and images as insets on a phylogenetic tree in *ggtreeExtra*. Subplots can be a *ggplot* object that generated by ggplot2 or its extensions that summarizes taxon-specific information (Fig. \ref{fig:subplot}.A). Taxon information can be used color or scale the size of silhouette images (Fig. \ref{fig:subplot}.B). The host tree of Fig. \ref{fig:subplot}.B was obtained from TimeTree [@TimeTree2017], the branch colour represents the Mantel Pearson correlation of gut microbiome, the stack bar chart represents the diet composition of the host, the inner ring represents the host taxonomic class, the middle ring represents flight status, and some representative species are also displayed in the outermost circle (Fig. \ref{fig:subplot}.B).
