...
If you want to draw the examples of the supplemental file, you also need to install the following packages. You can refer the following commands to install them.
...
# 'BiocManager::install("ggtree")' once the Bioconductor (>=3.13) was released.
...
# 'BiocManager::install(treeio)' once the Bioconductor (>=3.13) was released.
...
With ggtreeExtra incorporating all these information, some of the evolutionary patterns that are not straightforward might become more obvious. For example, we found some strains from Firmicutes and Gammaproteobacteria have more inhibitor interactions. However, many strains from Alphaproteobacteria and Betateobacteria prefer the interaction of sensitivity. After subsequent Mann-Whitney's test, the number of different interactions among these strains was confirmed to be significant (Fig. \ref{fig:compareNumber}). To our knowledge, there are no other software tools that can easily produce a figure like this (Fig. \ref{fig:ArabidopsisTree}) and the visualization do help us to explore the data and generate new insights as our findings were not revealed in the original paper [@Arabidopsis].
...
```{r, echo=FALSE, message=FALSE, warning=FALSE,  fig.width=6, fig.height=4, error=FALSE, fig.cap='(ref:compareNumber)', results='asis',compareNumber}
library(ggplot2)
library(dplyr)
library(magrittr)
library(MicrobiotaProcess)
...
intersectNums2 <- reshape2::dcast(intersectNums, Isolation + Taxa ~ Number, sum)

tipInterNums <- intersectNums2 %>%
                select(-c("Taxa")) %>%
                tibble::column_to_rownames(var="Isolation")
tipTaxa <- intersectNums2 %>%
           select(c("Isolation", "Taxa")) %>%
           tibble::column_to_rownames(var="Isolation")

FigS10 <- MicrobiotaProcess::ggbox(obj=tipInterNums,
                               sampleda=tipTaxa,
                               factorNames="Taxa",
                               indexNames=c("Inhibitor", "Sensitive")
          ) +
          theme(axis.text.x=element_blank())
FigS10
```
