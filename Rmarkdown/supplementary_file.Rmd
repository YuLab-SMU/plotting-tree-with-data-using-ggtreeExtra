...
(ref:comparison) Visualization methods for tree annotation supported by different software tools, circle: circular, inward circular, radial; rectangle: rectangular, slanted, ellipse, round rectangular.

```{r, echo=FALSE, message=FALSE, warning=FALSE,  fig.width=7.6, fig.height=3.28, error=FALSE, fig.cap='(ref:comparison)', results='asis', out.width = '50%', compare}
require(kableExtra)
...
yy <- yy %>% dplyr::full_join(yy2, by="V1") %>%
      dplyr::filter(!(V1=="ggtreeExtra" & value=="pie" & layout=="circle")) %>%
...
p1 <- ggplot() +
     ...
     geom_star(data=yy, aes(x=value, y=V1, fill=layout, starshape=layout),
               starstroke=0.3, size=5, color="grey68") +
     scale_fill_manual(
...
```{r, fig.width=8, fig.height=8, error=FALSE, warning=FALSE, message=FALSE, fig.cap='(ref:ggridges)', ggridges}
library(ggtree)
library(ggplot2)
library(ggtreeExtra)
library(patchwork)
...
```{r, fig.width=7, fig.height=5, error=FALSE, warning=FALSE, message=FALSE, fig.cap='(ref:ggpattern)', ggpattern}
library(ggtree)
library(ggtreeExtra)
library(ggpattern)
library(ggplot2)
